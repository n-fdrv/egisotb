...
from passengers.models import GENDER_CHOICES, Citizenship, DocType, Passenger
...
class Ferry(CreatedModel):
    ...
    registration_number = models.CharField(
        "Регистрационный номер судна", max_length=50, unique=True
    )
    ...
    ship_class = models.CharField(
        "Класс судна", max_length=1, choices=SHIP_CLASS_CHOICES
    )
...
class CrewMember(CreatedModel):
    ...
    patronymic = models.CharField(
        "Отчество", max_length=100, blank=True, null=True
    )
    ...
    doc_type = models.ForeignKey(
        DocType, on_delete=models.CASCADE, verbose_name="Вид документа"
    )
...
class Voyage(CreatedModel):
    ...
    route_type = models.CharField(
        "Тип маршрута", max_length=1, choices=ROUTE_TYPE_CHOICES
    )
