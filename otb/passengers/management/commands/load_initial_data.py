...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(
            "passengers/fixtures/countries.json", "r", encoding="utf-8"
        ) as f:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ {len(countries)} записей гражданства загружено"
                )
            )

        # Загрузка типов документов
        with open(
            "passengers/fixtures/doc_types.json", "r", encoding="utf-8"
        ) as f:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ {len(doc_types)} записей типов документов загружено"
                )
            )
