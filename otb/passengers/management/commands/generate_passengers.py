...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not citizenships:
            self.stdout.write(self.style.ERROR('Нет записей в Citizenship. Сначала загрузите справочник стран'))
            return

        if not doc_types:
            self.stdout.write(self.style.ERROR('Нет записей в DocType. Сначала загрузите типы документов'))
            return

        if not User.objects.exists():
            self.stdout.write(self.style.ERROR('Нет пользователей. Создайте хотя бы одного суперпользователя'))
            return
