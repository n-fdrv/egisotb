...
def passenger_list(request):
    if request.method == "GET":
        ...
        if doc_type == 1 and not re.fullmatch(r"^\d{10}$", doc_number):
            return Response(
                {
                    "error": "Неверный формат паспорта (должен состоять из 10 цифр)"
                },
                status=400,
            )
...
def passenger_detail(request, pk):
    ...
    if request.method == "GET":
        ...
        data = request.data.copy()
        data["created_by"] = (
            passenger.created_by.id if passenger.created_by else None
        )
        data["surname"] = data.get("surname", "").upper()
        data["name"] = data.get("name", "").upper()
        data["created_by"] = request.user.id  # Устанавливаем создателя
...
def unlock_schedule(request, pk):
    ...
    return Response({"detail": "Рейс разблокирован"})
