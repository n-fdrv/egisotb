...
from route.models import PassengerVoyage, Voyage, CrewVoyage, Ferry, CrewMember
...
def user_detail(request, pk):
    ...
    added_passengers_schedules = PassengerVoyage.objects.filter(created_by=user).values_list('voyage_id',
                                                                                             flat=True).count()
    added_crew_schedules = CrewVoyage.objects.filter(created_by=user).values_list('voyage_id',
                                                                                  flat=True).count()

    # --- Последние рейсы, где был пользователь ---
    recent_schedules = Voyage.objects.filter(created_by=user).select_related("ferry").order_by('-departure_date')[:30]
    ...
    schedule_passenger_actions = PassengerVoyage.objects.filter(created_by=user).select_related('passenger',
                                                                                                'voyage').values(
        'id', 'passenger__surname', 'passenger__name', 'voyage__name', 'created_at', 'voyage__departure_date', 'voyage__departure_time'
    )

    schedule_crew_actions = CrewVoyage.objects.filter(created_by=user).select_related('crew_member', 'voyage').values(
        'id', 'crew__surname', 'crew__name', 'voyage__name', 'created_at', 'voyage__departure_date', 'voyage__departure_time'
    )
    ...
    for sp in schedule_passenger_actions:
        logs.append({
            'time': sp['created_at'].strftime('%d.%m %H:%M'),
            'object_type': 'пассажир -',
    ...
    for sc in schedule_crew_actions:
        logs.append({
            'time': sc['created_at'].strftime('%d.%m %H:%M'),
            'object_type': 'экипаж - ',
    ...
    for f in ferry_actions:
        logs.append({
            'time': f['created_at'].strftime('%d.%m %H:%M'),
            'object_type': 'Паром',
            'object_repr': f['name'],
            'action': 'Создан',
            'schedule': ''
    ...
    logs.sort(key=lambda x: datetime.strptime(x['time'], '%d.%m %H:%M'), reverse=True)
    ...
    context = {
        ...
        'recent_schedules': [{
            ...
            'passenger_count': PassengerVoyage.objects.filter(voyage=s).count()
